const VimeoApi = {
  ...
  _createPlayer: (settings) => {
    ...
    window["__ready__".concat(settings.id)](player);
  },
  /**
...
const vimeoIframeRenderer = {
  ...
  create: (mediaElement, options, mediaFiles) => {
    const apiStack = [], vimeo = {}, readyState = 4;
    let paused = true, volume = 1, oldVolume = volume, currentTime = 0, bufferedTime = 0, ended = false, duration = 0, vimeoPlayer = null, url = "";
    vimeo.options = options;
    vimeo.id = mediaElement.id + "_" + options.prefix;
    vimeo.mediaElement = mediaElement;
    const errorHandler = (error) => {
      mediaElement.generateError("Code ".concat(error.name, ": ").concat(error.message), mediaFiles);
    };
    const props = mejs.html5media.properties, assignGettersSetters = (propName) => {
      const capName = "".concat(propName.substring(0, 1).toUpperCase()).concat(propName.substring(1));
      vimeo["get".concat(capName)] = () => {
        if (vimeoPlayer !== null) {
          const value = null;
          switch (propName) {
            case "currentTime":
      ...
      vimeo["set".concat(capName)] = (value) => {
        if (vimeoPlayer !== null) {
          switch (propName) {
            case "src":
              const url2 = typeof value === "string" ? value : value[0].src, videoId = VimeoApi.getVimeoId(url2);
            ...
            case "currentTime":
              vimeoPlayer.setCurrentTime(value).then(() => {
                currentTime = value;
                setTimeout(() => {
                  const event2 = mejs.Utils.createEvent("timeupdate", vimeo);
            ...
            case "volume":
              vimeoPlayer.setVolume(value).then(() => {
                ...
                oldVolume = volume;
                setTimeout(() => {
                  const event2 = mejs.Utils.createEvent("volumechange", vimeo);
                  mediaElement.dispatchEvent(event2);
                }, 50);
              }).catch((error) => errorHandler(error));
              break;
    ...
    const methods = mejs.html5media.methods, assignMethods = (methodName) => {
      vimeo[methodName] = () => {
        if (vimeoPlayer !== null) {
          switch (methodName) {
            case "play":
      ...
      if (apiStack.length) {
        for (let i = 0, total = apiStack.length; i < total; i++) {
          ...
          if (stackItem.type === "set") {
            const propName = stackItem.propName, capName = "".concat(propName.substring(0, 1).toUpperCase()).concat(propName.substring(1));
            vimeo["set".concat(capName)](stackItem.value);
    ...
    const height = mediaElement.originalNode.height, width = mediaElement.originalNode.width, vimeoContainer = document.createElement("iframe"), standardUrl = "https://player.vimeo.com/video/".concat(VimeoApi.getVimeoId(mediaFiles[0].src));
    let queryArgs = ~mediaFiles[0].src.indexOf("?") ? "?".concat(mediaFiles[0].src.slice(mediaFiles[0].src.indexOf("?") + 1)) : "";
    const args = [];
    ...
    queryArgs = "".concat(queryArgs).concat(queryArgs ? "&" : "?").concat(args.join("&"));
    vimeoContainer.setAttribute("id", vimeo.id);
    vimeoContainer.setAttribute("width", width);
    vimeoContainer.setAttribute("height", height);
    vimeoContainer.setAttribute("frameBorder", "0");
    vimeoContainer.setAttribute("src", "".concat(standardUrl).concat(queryArgs));
    ...
    vimeo.setSize = (width2, height2) => {
      vimeoContainer.setAttribute("width", width2);
      vimeoContainer.setAttribute("height", height2);
    };
    vimeo.show = () => {
      if (vimeoPlayer) {
