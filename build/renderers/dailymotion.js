const DailyMotionApi = {
  ...
  createIframe: (settings) => {
    ...
    player.addEventListener("apiready", () => {
      window["__ready__".concat(settings.id)](player, { paused: true, ended: false });
    });
  },
  /**
  ...
  getDailyMotionId: (url) => {
    const parts = url.split("/"), lastPart = parts[parts.length - 1], dashParts = lastPart.split("_");
    return dashParts[0];
  }
};
const DailyMotionIframeRenderer = {
  ...
  create: (mediaElement, options, mediaFiles) => {
    const dm = {}, apiStack = [], readyState = 4;
    let events, dmPlayer = null, dmIframe = null, muted = mediaElement.originalNode.muted;
    dm.options = options;
        ...
        if (dmPlayer !== null) {
          switch (propName) {
            ...
            case "muted":
              ...
              muted = value;
              setTimeout(() => {
                const event2 = mejs.Utils.createEvent("volumechange", dm);
      ...
      if (apiStack.length) {
        for (let i = 0, total = apiStack.length; i < total; i++) {
          ...
          if (stackItem.type === "set") {
            ...
            dm["set".concat(capName)](stackItem.value);
    ...
    const videoId = DailyMotionApi.getDailyMotionId(mediaFiles[0].src), dmSettings = {
      id: dm.id,
