const TwitchApi = {
  ...
  getTwitchIdFromParam: (url) => {
    ...
    for (let i = 0, total = parameters.length; i < total; i++) {
      ...
      if (~paramParts[0].indexOf("channel")) {
        ...
        twitchId = "v".concat(paramParts[1]);
        break;
      }
    }
    return twitchId;
  ...
  getTwitchIdFromUrl: (url) => {
    ...
    return /^\d+$/i.test(id) ? "v".concat(id) : id;
  },
  /**
...
const TwitchIframeRenderer = {
  ...
  create: (mediaElement, options, mediaFiles) => {
    const twitch = {}, apiStack = [], readyState = 4, twitchId = TwitchApi.getTwitchId(mediaFiles[0].src);
    let twitchPlayer = null, paused = true, ended = false, volume = 1, duration = Infinity, time = 0;
    ...
    const props = mejs.html5media.properties, assignGettersSetters = (propName) => {
      ...
      twitch["set".concat(capName)] = (value) => {
        if (twitchPlayer !== null) {
          switch (propName) {
            case "src":
              const url = typeof value === "string" ? value : value[0].src, videoId = TwitchApi.getTwitchId(url);
            ...
            case "currentTime":
              ...
              setTimeout(() => {
                const event2 = mejs.Utils.createEvent("timeupdate", twitch);
            ...
            case "muted":
              ...
              setTimeout(() => {
                const event2 = mejs.Utils.createEvent("volumechange", twitch);
                mediaElement.dispatchEvent(event2);
              }, 50);
              break;
            case "volume":
    ...
    window["__ready__" + twitch.id] = (_twitchPlayer) => {
      ...
      if (apiStack.length) {
        for (let i = 0, total = apiStack.length; i < total; i++) {
          ...
          if (stackItem.type === "set") {
            ...
            twitch["set".concat(capName)](stackItem.value);
      ...
      const events = ["mouseover", "mouseout"], assignEvents = (e) => {
    ...
    const height = mediaElement.originalNode.height, width = mediaElement.originalNode.width, twitchContainer = document.createElement("div"), type = TwitchApi.getTwitchType(twitchId), twitchSettings = {
      id: twitch.id,
      width,
      height,
      playsinline: false,
    ...
    twitch.setSize = (width2, height2) => {
      if (TwitchApi !== null && !isNaN(width2) && !isNaN(height2)) {
        twitchContainer.setAttribute("width", width2);
        twitchContainer.setAttribute("height", height2);
      }
    };
    twitch.hide = () => {
      twitch.pause();
