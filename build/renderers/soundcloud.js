...
const SoundCloudIframeRenderer = {
  ...
  create: (mediaElement, options, mediaFiles) => {
    const sc = {}, apiStack = [], readyState = 4, autoplay = mediaElement.originalNode.autoplay, isVideo = mediaElement.originalNode !== null && mediaElement.originalNode.tagName.toLowerCase() === "video";
    let duration = 0, currentTime = 0, bufferedTime = 0, volume = 1, muted = false, paused = true, ended = false, scPlayer = null, scIframe = null;
    sc.options = options;
        ...
        if (scPlayer !== null) {
          switch (propName) {
            ...
            case "muted":
              ...
              setTimeout(() => {
                const event2 = mejs.Utils.createEvent("volumechange", sc);
      ...
      if (apiStack.length) {
        for (let i = 0, total = apiStack.length; i < total; i++) {
          ...
          if (stackItem.type === "set") {
            ...
            sc["set".concat(capName)](stackItem.value);
