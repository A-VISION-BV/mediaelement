import { w as window, t as typeChecks, r as renderer, o as loadScript, m as mejs, c as createEvent } from "../dom.js";
...
const YouTubeIframeRenderer = {
  ...
  create: (mediaElement, options, mediaFiles) => {
    ...
    const props = mejs.html5media.properties, assignGettersSetters = (propName) => {
      ...
      youtube["set".concat(capName)] = (value) => {
        if (youTubeApi !== null) {
          switch (propName) {
            case "src":
    ...
    const errorHandler = (error) => {
      ...
      mediaElement.generateError("Code ".concat(error.data, ": ").concat(message), mediaFiles);
    };
          ...
          if (apiStack.length) {
            for (let i = 0, total = apiStack.length; i < total; i++) {
              ...
              if (stackItem.type === "set") {
                ...
                youtube["set".concat(capName)](stackItem.value);
    ...
    youtube.getPosterUrl = () => {
      ...
      return quality && resolutions.indexOf(quality) > -1 && id ? "https://img.youtube.com/vi/".concat(id, "/").concat(quality, ".jpg") : "";
    };
    return youtube;
  }
};
