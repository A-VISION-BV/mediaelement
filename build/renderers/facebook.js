const FacebookApi = {
  ...
  load: (settings) => {
    if (typeof FB !== "undefined") {
      ...
      FacebookApi.promise = FacebookApi.promise || mejs.Utils.loadScript("https://connect.facebook.net/".concat(settings.options.lang, "/sdk.js"));
  ...
  _createPlayer: (settings) => {
    window["__ready__".concat(settings.id)]();
  }
};
const FacebookRenderer = {
  ...
  create: (mediaElement, options, mediaFiles) => {
    const apiStack = [], fb = {}, readyState = 4;
    let paused = true, ended = false, fbPlayer = null, src = "", poster = "", autoplay = mediaElement.originalNode.autoplay;
    fb.options = options;
        ...
        if (fbPlayer !== null) {
          switch (propName) {
            ...
            case "muted":
              ...
              setTimeout(() => {
                const event2 = mejs.Utils.createEvent("volumechange", fb);
    ...
    window["__ready__" + fb.id] = () => {
      FB.Event.subscribe("xfbml.ready", (msg) => {
        if (msg.type === "video" && fb.id === msg.id) {
          ...
          const fbIframe = document.getElementById(fb.id), width = fbIframe.offsetWidth, height = fbIframe.offsetHeight, events = ["mouseover", "mouseout"], assignIframeEvents = (e) => {
          ...
          for (let i = 0, total = fbEvents.length; i < total; i++) {
            const event = fbEvents[i], handler = fb.eventHandler[event];
            if (handler !== void 0 && handler !== null && !mejs.Utils.isObjectEmpty(handler) && typeof handler.removeListener === "function") {
          ...
          if (apiStack.length) {
            for (let i = 0, total = apiStack.length; i < total; i++) {
              ...
              if (stackItem.type === "set") {
                ...
                fb["set".concat(capName)](stackItem.value);
